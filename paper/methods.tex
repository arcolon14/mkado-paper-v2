...
following the \texttt{libsequence} convention.
...
% Note: I think the applications only allow for one figure?
% https://academic.oup.com/bioinformatics/pages/author-guidelines#section-4
\begin{figure}[!t]
  \centering
...
% Since the text talks about this and the asymptotic plot, maybe
% having Fig2 be a A + B panel with both?
\begin{figure}[!t]
  \centering
  \includegraphics[width=0.85\columnwidth]{figures/mk_volcano_pongo.png}
  \caption{Volcano plot from standard MK tests on 12,437 genes using human 
    polymorphism data, using \textit{Pongo abelii} as the outgroup for divergence.
    Red points are significant after Bonferroni correction, while
    orange points are nominally significant ($p < 0.05$). The
